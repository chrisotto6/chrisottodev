...
with [Netlify](https://www.netlify.com/). Originally I tried handrolling my own site but it was taking way too long to get everything I wanted
...
to work with both MDX and MD to start out and to use my templates that I already had.
...
If you have your setup setup to publish out an RSS feed you will need to switch to this package, this is the other plugin the
`gatsby-config.js` that we need to rename. Adjust your feed query to be from `allMarkdownRemark` to `allMdx` and instead of referencing the results `html` the MDX plugin
uses a `body` field.
...
want otherise keep the options for extensions the same and they should render correctly. Now it's time to remove the packages that are
...
There a few gotchas along the way that tripped me up for a little bit and may be specific to the way I have things setup but wanted to
call them out here.
